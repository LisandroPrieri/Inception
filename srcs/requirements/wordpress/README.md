...
- **First `RUN`**: one layer doing update + install + cleanup, `&&`-chained so a failure stops the build. The packages are the ones explained above, plus `curl` (used by the next instruction) and `mariadb-client` (used by `init.sh` at runtime to poll the database).
