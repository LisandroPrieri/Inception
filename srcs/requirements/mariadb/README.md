...
- **Temporary server**: SQL needs a running server, but the final server must be the foreground PID 1 process. So: start in the background (`&`), wait for readiness, run the SQL, shut down cleanly, then start for real. `--user=mysql` is required because `mariadbd` launched by root refuses to run unless told which unprivileged account to become. **`--skip-networking`** confines this server to its Unix socket, binding no TCP port: every setup step here is local and reaches it through `/run/mysqld/mysqld.sock`, while WordPress, which polls `mariadb:3306` over the network, cannot mistake this short-lived server for the real one and start installing against a database that is about to shut down (see Issues below).
...
- **WordPress installed against the temporary server and died mid-install**: on a genuine first boot, `wp core install` failed with `Error establishing a database connection`, three seconds after the WordPress readiness poll had succeeded. The setup server started here listened on TCP as well as its socket, so WordPress's `until mariadb -h mariadb ...` loop connected to *it*, declared the database ready, and then ran the install into the one-second window between `Shutdown complete` and the real server's `ready for connections`. Fixed with `--skip-networking` on the temporary server, which leaves it reachable only through the local socket, so nothing on the bridge network can connect before the real server is up. The lesson generalises: a readiness probe must be unable to answer "yes" for the wrong server.
