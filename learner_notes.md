...
- **Keywords are the grammar**: fixed, they come from the spec.
...
| `php-mbstring` | Multibyte-safe string functions | UTF-8. `strlen("é")` is wrong without it; it breaks accents, emoji, non-Latin scripts |
...
| Attempt | Result |
|---|---|
| `mysql -u root` as OS **root**, over the socket | `root@localhost`, logged in |
